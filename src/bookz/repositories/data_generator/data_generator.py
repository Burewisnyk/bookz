...
def generate_fake_authors(quantity: int) -> list[Author]:
    ...
    for _ in range(quantity):
        ...
        if sex == "M":
            ...
            middle_name = (
                fake_gen.first_name_male()
                + fake_gen.random_element(male_patronymic_suffixes)
                if fake_gen.boolean(chance_of_getting_true=70)
                else None
            )
        else:
            ...
            middle_name = (
                fake_gen.first_name_male()
                + fake_gen.random_element(female_patronymic_suffixes)
                if fake_gen.boolean(chance_of_getting_true=70)
                else None
            )
        author = Author(
...
def generate_fake_books(quantity: int) -> list[Book]:
    ...
    for _ in range(quantity):
        ...
        price = (
            ...
            if fake_gen.boolean(chance_of_getting_true=80)
            else None
        )
        language = (
            fake_gen.language_code()
            if fake_gen.boolean(chance_of_getting_true=70)
            else None
        )
        faker_books.append(
