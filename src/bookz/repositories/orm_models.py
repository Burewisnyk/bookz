...
class Author(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "first_name", "last_name", "middle_name", name="uq_author_full_name"
        ),
    ...
    def __repr__(self) -> str:
        return (
            f"Author(id={self.id}, first_name='{self.first_name}', "
...
class Book(Base, TimestampMixin):
    ...
    def __repr__(self) -> str:
        return (
            f"Book(id={self.book_id}, title='{self.title}', publisher='{self.publisher}', "
...
class BookCopy(Base, TimestampMixin):
    ...
    status: Mapped[BookStatus] = mapped_column(
        PgEnum(BookStatus, name="book_copy_status"),
        nullable=False,
        default=BookStatus.AVAILABLE,
    )
    placement_id: Mapped[int | None] = mapped_column(
        ForeignKey("placements.id"), default=None
    )
    statement: Mapped[BookStatement] = mapped_column(
        PgEnum(BookStatement), nullable=False, default=BookStatement.NEW
    )
    customer_id: Mapped[int | None] = mapped_column(
        ForeignKey("customers.customer_id"), default=None
    )

    book: Mapped[Book] = relationship("Book", back_populates="book_copies")
    customer: Mapped[Customer] = relationship(
        "Customer", back_populates="borrowed_books"
    )
    placement: Mapped[Placement] = relationship("Placement", back_populates="book_copy")

    def __repr__(self) -> str:
        return (
            f"BookCopy(id={self.copy_id}, book={self.book}, status='{self.status.value}', "
...
class Customer(Base, TimestampMixin):
    ...
    borrowed_books: Mapped[list[BookCopy]] = relationship(
        "BookCopy", back_populates="customer"
    )

    __table_args__ = (
        Index("ix_customer_full_name", "last_name", "first_name", "middle_name"),
        UniqueConstraint(
            "last_name", "first_name", "middle_name", name="uq_customer_full_name"
        ),
    ...
    def __repr__(self) -> str:
        return (
            f"Customer(customer_id={self.customer_id}, last_name='{self.last_name}', "
...
class Placement(Base, TimestampMixin):
    ...
    status: Mapped[PlacementStatus] = mapped_column(
        PgEnum(PlacementStatus, name="placement_status"),
        nullable=False,
    ...
    def __repr__(self) -> str:
        return (
            f"Placement(id={self.id}, line_id='{self.line_id}', column_id={self.column_id}, "
