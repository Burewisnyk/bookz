...
def class_constructor(loader, node):
    ...
    try:
        ...
        app_logger.error(
            f"Error parsing yaml file, !class '{class_name}' that declarated in yaml configuration "
...
class CustomORMMapper:
    ...
    @staticmethod
    def map_recursively(
        orm_instance, config: dict, max_depth: int = 5, _current_depth: int = 0
    ):
        ...
        if isinstance(orm_instance, list):
            return [
                CustomORMMapper.map_recursively(item, config, max_depth, _current_depth)
                for item in orm_instance
            ]
        ...
        for field_name, field_info in dto_class.model_fields.items():
            ...
            if (
                "nested_transform" in config
                and field_name in config["nested_transform"]
            ):
...
class AuthorMapper(CustomORMMapper):
    ...
    def dto_to_dict(author: AuthorDTO) -> dict:
        app_logger.debug(
            f"Call AuthorMapper class method dto_to_dict with parameters: {author}"
        )
        ...
        return {k: v for k, v in author_dict.items() if k in author_columns}

    @staticmethod
    def new_dto_to_dict(author: NewAuthorDTO) -> dict:
        app_logger.debug(
            f"Call AuthorMapper class method new_dto_to_dict with parameters: {author}"
        )
        author_columns = Author.__table__.columns.keys()
        author_dict = author.model_dump()
        full_name = author_dict.pop("full_name", {})
        ...
        return {
            k: v
            for k, v in author.model_dump(exclude_unset=True).items()
    ...
    def orm_to_dto(author: Author) -> AuthorDTO:
        app_logger.debug(
            f"Call AuthorMapper class method orm_to_dto with parameters: {author}"
        )
        return AuthorMapper.map_recursively(
            orm_instance=author, config=AuthorMapper.author_config
        )
...
class BookMapper(CustomORMMapper):
    ...
    def dto_to_dict(book: BookDTO) -> dict:
        app_logger.debug(
            f"Call BookMapper class method dto_to_dict with parameters: {book}"
        )
    ...
    def new_dto_to_dict(book: NewBookDTO) -> dict:
        app_logger.debug(
            f"Call BookMapper class method new_dto_dict with parameters: {book}"
        )
        book_columns = Book.__table__.columns.keys()
        return {
            k: v
    ...
    def orm_to_dto(book: Book) -> BookDTO:
        app_logger.debug(
            f"Call BookMapper class method orm_to_dto with parameters: {book}"
        )
        return BookMapper.map_recursively(
            orm_instance=book, config=BookMapper.book_config
        )
...
class BookCopyMapper(CustomORMMapper):
    ...
    def dto_to_dict(book: BookCopyDTO) -> dict:
        app_logger.debug(
            f"Call BookCopyMapper class method dto_to_dict with parameters: {book}"
        )
        book_columns = Book.__table__.columns.keys()
        return {
            k: v
    ...
    def new_dto_to_dict(book: NewBookCopyDTO) -> dict:
        app_logger.debug(
            f"Call BookCopyMapper class method new_dto_to_dict with parameters: {book}"
        )
        book_columns = Book.__table__.columns.keys()
        return {
            k: v
    ...
    def orm_to_dto(book: BookCopy) -> BookCopyDTO:
        app_logger.debug(
            f"Call BookCopyMapper class method orm_to_dto with parameters: {book}"
        )
        return BookCopyMapper.map_recursively(
            orm_instance=book, config=BookCopyMapper.book_copy_config
        )
...
class CustomerMapper(CustomORMMapper):
    ...
    def dto_to_dict(customer: CustomerDTO) -> dict:
        app_logger.debug(
            f"Call CustomerMapper class method dto_to_dict with parameters: {customer}"
        )
        ...
        return {k: v for k, v in customer_dict.items() if k in customer_columns}

    @staticmethod
    def new_dto_to_dict(customer: NewCustomerDTO) -> dict:
        app_logger.debug(
            f"Call CustomerMapper class method new_dto_to_dict with parameters: {customer}"
        )
        customer_columns = Customer.__table__.columns.keys()
        customer_dict = customer.model_dump(exclude_unset=True)
        full_name = customer_dict.pop("full_name", {})
    ...
    def orm_to_dto(customer: Customer) -> CustomerDTO:
        app_logger.debug(
            f"Call CustomerMapper class method orm_to_dto with parameters: {customer}"
        )
        return CustomerMapper.map_recursively(
            orm_instance=customer, config=CustomerMapper.customer_config
        )
...
class FullNameMapper:
    ...
    def dto_to_dict(full_name: FullNameDTO) -> dict:
        app_logger.debug(
            f"Call FullNameMapper class method dto_to_dict with parameters: {full_name}"
        )
...
class PhoneMapper:
    ...
    def phone_number_to_united_style(phone: str) -> str:
        ...
        app_logger.debug(
            f"Call PhoneMapper class method phone_number_to_united_style with parameters: {phone}"
        )
        ...
        return "+" + "".join(
            (ch + " ") if i in SPACES else ch for i, ch in enumerate(phone_number)
        )
