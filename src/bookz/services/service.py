...
class BookService:
    ...
    def find_author_by_id(self, author_id: int) -> AuthorDTO:
        app_logger.info(
            f"Calling find_author_by_id function with parameter: {author_id}"
        )
        ...
        app_logger.info(
            f"Author:{author}, books: {author.books}, {author.books[0].book_copies}"
        )
        return AuthorMapper.orm_to_dto(author)

    def find_author_by_full_name(self, author: FullNameDTO) -> AuthorDTO:
        app_logger.info(
            f"Calling find_author_by_full_name function with parameter: {author}"
        )
    ...
    def create_author(self, author: NewAuthorDTO) -> AuthorDTO:
        ...
        try:
            ...
            if (
                ...
                and orig.diag.constraint_name == "uq_author_full_name"
            ):
                app_logger.warning(
                    f"Author with full name {str(author.full_name)} already exists in database"
                )
                raise AuthorFullNameAlreadyExist(
                    f"Author with full name: {str(author.full_name)} is already exist "
                    f"in database"
                ) from e
            else:
                raise e
    ...
    def update_author(self, author: AuthorDTO) -> AuthorDTO:
        ...
        try:
            ...
            if (
                ...
                raise AuthorFullNameAlreadyExist(
                    f"Author with fullname {str(author.full_name)} is "
                    f"already exist in database"
                ) from e

    def delete_author_by_id(self, author_id: int) -> AuthorDTO:
        app_logger.info(
            f"Calling delete_author_by_id function with parameter: {author_id}"
        )
        book_author = self.repo.find_books_by_author_id(author_id)
        if book_author:
            app_logger.warning(f"Author with id {author_id} not found")
            raise BookPresentInDatabase(
                f"{len(book_author)} book(s) of this author is present in repository. "
                f"Delete author books before deleting the author"
            )
    ...
    def create_book(self, book: NewBookDTO) -> BookDTO:
        ...
        if present_book:
            app_logger.warning(
                f"Book not created. Book with isbn {book.isbn} present in database"
            )
            raise BookPresentInDatabase(
                f"Book with isbn {book.isbn} present in database"
            )
        try:
            with self.session.begin():
                ...
                for author in book.authors:
                    ...
                    if not ath:
                        ath = self.repo.create_author(
                            AuthorMapper.new_dto_to_dict(author)
                        )
                ...
                if len(available_places) < book.new_copies:
                    raise StorageSpaceIsNotSufficient(
                        f"Free place for {book.new_copies} of {book.title} is`t available "
                    )
                new_book_copies: list[dict] = []
                for _ in book.copies:
                    place = available_places.pop()
                    new_book_copies.append(
                        BookCopyMapper.new_dto_to_dict(
                            NewBookCopyDTO(
                                book_id=book.id,
                ...
                self.repo.change_places_status(
                    place_ids=places, status=PlacementStatus.OCCUPIED
                )
        except IntegrityError as e:
            orig = e.orig
            if (
                isinstance(orig, IntegrityError)
                and orig.diag.constraint_name == "uq_isbn"
            ):
    ...
    def delete_book(self, book_id: int) -> BookDTO:
        ...
        with self.session.begin():
            ...
            if borrowed_copies_ids:
                app_logger.warning(
                    f"Attempt to delete book {book.title} with isbn {book.isbn} whose copies "
                    f"{borrowed_copies_ids} are borrowed"
                )
                raise BookCopyBorrowed(
                    f"Book copies {borrowed_copies_ids}  of book '{book.title}' is borrowed. "
                    f"Change its status before delete"
                )
            self.repo.delete_book_copies_by_ids(ids=copy_ids)
            self.repo.change_places_status(
                place_ids=place_ids, status=PlacementStatus.FREE
            )
    ...
    def find_book_copies_for_status(self, status: BookStatus) -> list[BookCopyDTO]:
        app_logger.info(
            f"Calling find_book_copies_for_status function with parameter: {status}"
        )
    ...
    def find_book_copies_for_statement(
        self, statement: BookStatement
    ) -> list[BookCopyDTO]:
        app_logger.info(
            f"Calling find_book_copies_for_statement function with parameter: {statement}"
        )
        ...
        if not book_copies:
            ...
            raise BookCopyNotFound(
                f"Book copy(ies) with statement {statement} not found"
            )
        copies = []
        for book_copy in book_copies:
            copies.append(BookCopyMapper.orm_to_dto(book_copy))
    ...
    def create_book_copy(self, book_copy: NewBookCopyDTO) -> BookCopyDTO:
        app_logger.info(
            f"Calling create_book_copy function with parameter: {book_copy}"
        )
        with self.session.begin():
            place = self.repo.find_free_place(1)
            if not place:
                app_logger.warning(
                    "Free place in depository for new book copy not found"
                )
                raise StorageSpaceIsNotSufficient(
                    f"Free place in depository for new book copy is`t available "
                )
            book = self.repo.find_book_by_id(book_copy.book_id, for_update=True)
            if not book:
                app_logger.warning(
                    f"Book with book_id: {book_copy.book_id} for this book copy not found"
                )
                raise BookNotFound(
                    f"Book for this book copy not found. Add first the book"
                )
            book_copy.placement = place
            book_copy = self.repo.create_book_copy(
                BookCopyMapper.new_dto_to_dict(book_copy)
            )
            self.repo.change_place_status(
                place_id=place.pop(), status=PlacementStatus.OCCUPIED
            )
            book_copy = self.repo.find_book_by_id(book_copy.copy_id)  # type: ignore
        return BookCopyMapper.orm_to_dto(book_copy)

    def change_book_copy_status(
        self, copy_id: int, status: BookStatus, customer_id: int | None = None
    ) -> BookCopyDTO:
        app_logger.info(
            f"Calling change_book_copy_status function with parameter: copy_id: {copy_id}, "
            f"status: {status}, customer_id: {customer_id}"
        )
        if status == BookStatus.BORROWED and not customer_id:
            app_logger.warning(
                f"Bad function parameters. For status BORROWED customer_id must be not NULL"
            )
            raise CustomerMustBeGiven(
                f"When book copy is borrowed, customer id is required"
            )
        ...
        with self.session.begin():
            if status == BookStatus.AVAILABLE:
                ...
                if not place:
                    app_logger.warning(
                        "Free place in depository for new book copy is`t available "
                    )
                    raise StorageSpaceIsNotSufficient(
                        f"Free place in depository for book copy is`t available "
                    )
                self.repo.change_place_status(
                    place_id=place.pop(), status=PlacementStatus.OCCUPIED
                )
                book_copy = self.repo.update_book_copy(
                    copy_id=copy_id, book_copy={"status": status, "customer": None}
                )
                return BookCopyMapper.orm_to_dto(book_copy)
                ...
                book_copy = self.repo.update_book_copy(
                    copy_id=copy_id,
                    book_copy={"status": status, "customer": customer_id},
                )
                return BookCopyMapper.orm_to_dto(book_copy)
            else:
                book_copy = self.repo.update_book_copy(
    ...
    def change_book_copy_statement(
        self, copy_id: int, statement: BookStatement
    ) -> BookCopyDTO:
        app_logger.info(
            f"Calling change_book_copy_statement function with parameter: copy_id: {copy_id}, "
        ...
        with self.session.begin():
            ...
            if (
                (current_statement == BookStatement.NEW)
                or (
                    current_statement == BookStatement.GOOD
                    and statement != BookStatement.NEW
                )
                or (
                    (
                        current_statement == BookStatement.DAMAGED
                        or current_statement == BookStatement.UNUSABLE
                    )
                    and statement == BookStatement.REPAIR
                )
                or (
                    current_statement == BookStatement.DAMAGED
                    and statement == BookStatement.UNUSABLE
                )
            ):
                new_book_copy = self.repo.update_book_copy(
                    copy_id=copy_id, book_copy={"statement": statement}
                )
                return BookCopyMapper.orm_to_dto(new_book_copy)
            else:
                app_logger.warning(
                    f"Bad new statement {statement} for current book copy statement {current_statement}"
                )
                raise WrongNewStatement(
                    f"Book copy with id {copy_id} with current statement {current_statement} do "
                    f"not might be changed for new statement: {statement}"
                )

    def delete_book_copy(self, copy_id: int) -> BookCopyDTO:
        app_logger.info(
            f"Calling delete_book_copy function with parameter: copy_id: {copy_id}"
        )
        with self.session.begin():
            ...
            if delete_book_copy.satus == BookStatus.BORROWED:
                app_logger.warning(
                    f"Book with copy_id: {copy_id} borrowed. You cannot delete it"
                )
                raise BookCopyBorrowed(
                    f"You cannot delete book copy when it is borrowed. First change status"
                )
    ...
    def find_customer_by_id(self, cust_id: int) -> CustomerDTO:
        app_logger.info(
            f"Calling find_customer_by_id function with parameter: copy_id: {cust_id}"
        )
    ...
    def find_customer_by_email(self, email: str) -> CustomerDTO:
        app_logger.info(
            f"Calling find_customer_by_email function with parameter: email: {email}"
        )
    ...
    def find_customer_by_phone(self, phone: str) -> CustomerDTO:
        app_logger.info(
            f"Calling find_customer_by_phone function with parameter: {phone}"
        )
    ...
    def find_customer_by_fullname(self, fullname: FullNameDTO) -> CustomerDTO:
        app_logger.info(
            f"Calling find_customer_by_fullname function with parameter: {fullname}"
        )
    ...
    def create_customer(self, customer: NewCustomerDTO) -> CustomerDTO:
        ...
        try:
            customer = self.repo.create_customer(
                CustomerMapper.new_dto_to_dict(customer=customer)
            )
            return CustomerMapper.orm_to_dto(customer)
        except IntegrityError as e:
            ...
            if isinstance(original_error, UniqueViolation):
                ...
                if constraint_name == "uq_email":
                    ...
                    raise CustomerEmailAlreadyExist(
                        f"Customer with email: {customer.email} is already exist "
                        f"in database"
                    ) from e
                    ...
                    raise CustomerPhoneAlreadyExist(
                        f"Customer with phone: {customer.phone} is already exist "
                        f"in database"
                    ) from e
                elif constraint_name == "uq_customer_full_name":
                    app_logger.warning(
                        f"Customer with full name: {customer.fullname} already exists"
                    )
                    raise CustomerFullNameAlreadyExist(
                        f"Customer with fullname {str(customer.full_name)} is "
                        f"already exist in database"
                    ) from e
                else:
                    raise e
    ...
    def change_customer_phone_number(self, customer_id: int, phone: StringDTO):
        app_logger.info(
            f"Calling change_customer_phone_number function with parameter: {phone}"
        )
        phone = phone.string
        ...
        try:
            with self.session.begin():
                customer = self.repo.update_customer(
                    customer_id=customer_id, new_customer={"phone": phone}
                )
            ...
            if (
                isinstance(orig, UniqueViolation)
                and orig.diag.constraint_name == "uq_phone"
            ):
                app_logger.warning(f"Customer with phone: {phone} already exists")
                raise CustomerPhoneAlreadyExist(
                    f"Customer with phone: {customer.phone} is already exist in "
                    f"database"
                ) from e
            else:
                raise e
    ...
    def change_customer_email(self, customer_id: int, email: StringDTO) -> CustomerDTO:
        app_logger.info(
            f"Calling change_customer_email function with parameter: {email}"
        )
        email = email.string
        try:
            with self.session.begin():
                ...
                self.repo.update_customer(
                    customer_id=customer_id, new_customer={"email": email}
                )
                customer = self.repo.find_customer_by_id(customer_id)
            return CustomerMapper.orm_to_dto(customer)
        except IntegrityError as e:
            ...
            if (
                isinstance(orig, UniqueViolation)
                and orig.diag.constraint_name == "uq_email"
            ):
                app_logger.warning(
                    f"Customer with email: {email} already exists in database"
                )
                raise CustomerEmailAlreadyExist(
                    f"Customer with email: {customer.email} is already exist "
                    f"in database"
