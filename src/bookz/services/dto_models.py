...
class AuthorDTO(BaseModel):
    id: int = Field(
        ..., ge=0, description="Author id in database", examples=[1078, 204567]
    )
    full_name: FullNameDTO = Field(
        ...,
        description="Author full name. Must be unique",
        examples=[{"first_name": "Mark", "last_name": "Twen", "middle_name": None}],
    )
    books: list["BookDTO"] | None = Field(
        None, description="List of books in library that this author has"
    )
...
class NewAuthorDTO(BaseModel):
    full_name: FullNameDTO = Field(
        ...,
        description="New author full name. Must be unique",
        examples=[{"first_name": "", "last_name": "Savchu", "middle_name": None}],
    )
...
class PlacementDTO(BaseModel):
    ...
    def position_code(self) -> str:
        return (
            self.line_id + str(self.column_id) + self.shelf_id + str(self.position)
        ).upper()
...
class BookCopyDTO(BaseModel):
    ...
    status: BookStatus | None = Field(
        BookStatus.UNKNOWN, examples=[BookStatus.AVAILABLE, BookStatus.BORROWED]
    )
    statement: BookStatement | None = Field(
        BookStatement.NEW, examples=[BookStatement.NEW, BookStatement.REPAIR]
    )
    placement: PlacementDTO | None = Field(
        None,
        examples=[
            {
                "id": 578,
    ...
    customer: CustomerDTO | None = Field(
        None,
        examples=[
            None,
            {
...
class BookDTO(BaseModel):
    ...
    place_of_publication: str | None = Field(
        None, min_length=3, max_length=80, examples=["London"]
    )
    published_year: int | None = Field(None, ge=1700, le=2100, examples=[2008])
    isbn: str | None = Field(
        None, min_length=10, max_length=20, examples=["978-01-41-036144"]
    )
    ...
    authors: list["AuthorDTO"] = Field(
        None,
        examples=[
            {
                "id": 48,
                "full_name": {"first_name": "George", "last_name": "Orwell"},
                "books": None,
            }
        ],
    )
    book_copies: list[BookCopyDTO] | None = Field(
        None,
        examples=[
            {
                "id": 8217,
