    ...
    try:
        with get_session() as session:
            ...
            for book_id in book_ids:
                num_of_authors = random.choices([1, 2, 3, 4], weights=[60, 25, 10, 5])[
                    0
                ]
            ...
            placements = session.scalars(
                select(Placement.id).where(Placement.status == PlacementStatus.FREE)
            ).all()
            ...
            for book_id in book_ids:
                ...
                for copy in range(num_of_copies):  # type: ignore
                    status = random.choices(
                        [BookStatus.AVAILABLE, BookStatus.BORROWED, BookStatus.LOST],
                        weights=(70, 25, 5),
                    )[0]
                    statement = random.choices(
                        [
                            BookStatement.NEW,
                            BookStatement.GOOD,
                            BookStatement.REPAIR,
                            BookStatement.DAMAGED,
                        ],
                        weights=(20, 50, 20, 10),
                    )[0]
                    if status == BookStatus.AVAILABLE:
                        placement = placements.pop(
                            random.randint(0, len(placements) - 1)
                        )
                        session.add(
                            BookCopy(
                                book_id=book_id,
                        ...
                        session.add(
                            BookCopy(
                                book_id=book_id, status=status, statement=statement
                            )
                        )
            session.commit()
    except InterfaceError as e:
        app_logger.error(
            f"InterfaceError for database initialization. Error type {e.__class__.__name__}. "
        ...
        app_logger.error(
            f"DatabaseError for database initialization. Error type {e.__class__.__name__}. "
            f"Error message: {str(e)}"
        )
        db_logger.error(
            f"DatabaseError for database initialization. Error type {e.__class__.__name__}. "
        ...
        app_logger.critical(
            f"Unexpected error. Error type {e.__class__.__name__}. Error message: {str(e)}"
        )
